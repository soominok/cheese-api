...
class UserDfo:
    ...
    def new(self):
        ...
        # user_data = pd.concat([user_no_df, UserDfo.make_name()], axis = 1)
        ...
        # print(f'######## 중복값 확인 ##########')
        # a = this.user.duplicated()
        # a.to_csv('com_cheese_api/usr/user/data/duplicate.csv')

        
        self.odf = pd.DataFrame(
            {
                # 'user_no': this.user.order_no,
...
####################### 데이터 불러오기 & 생성 & featrue 제거 #######################
    ...
    # @staticmethod
    # def unique_id(this) -> object:
    #     id_list = this.user['user_id'].unique
    #     id_df = pd.DataFrame({'user_id': id_list})
    #     this.user = pd.merge(id_df, this.user, on = 'user_id', how = 'left')
    #     return this

    # @staticmethod
    # def make_user_no(this) -> object:
    #     num_list = []
    #     for num in range(40000):
    #         user_num = num 
    #         num_list.append(user_num)
    #     user_no_df = pd.DataFrame(num_list)
    #     this['user_no'] = user_no_df
    #     return this
    ...
    #     phone_list = []
    #     for num in range(40000):
    #         rand_phone = '010' + '-' + str(random.randrange(1000, 10000, 4))  + '-' + str(random.randrange(1000, 10000, 4)) 
